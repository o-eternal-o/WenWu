...
public class UserVerificationDAO {
    ...
    public void addVerification(UserVerification_Bean bean) throws Exception {
        String query = "INSERT INTO user_verification (user_id, real_name, id_type, id_number, status, submitted_at, reviewed_at, reviewed_by, reject_reason) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        dbConnector.executeUpdate(query,
                bean.getUserId(), bean.getRealName(), bean.getIdType(), bean.getIdNumber(),
    ...
    public void updateVerification(UserVerification_Bean bean) throws Exception {
        String query = "UPDATE user_verification SET user_id=?, real_name=?, id_type=?, id_number=?, status=?, submitted_at=?, reviewed_at=?, reviewed_by=?, reject_reason=? WHERE verification_id=?";
        dbConnector.executeUpdate(query,
                bean.getUserId(), bean.getRealName(), bean.getIdType(), bean.getIdNumber(),
                bean.getStatus(), bean.getSubmittedAt(), bean.getReviewedAt(), bean.getReviewedBy(), bean.getRejectReason(),
    ...
    private UserVerification_Bean mapBean(ResultSet rs) throws Exception {
        ...
        bean.setIdType(rs.getString("id_type"));
